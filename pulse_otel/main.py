...
def _register_baggage_span_processor():
    """Attach the baggage→span processor to the tracer provider Traceloop set as
    global, so every span (not just library-tagged ones) gets identity from baggage."""
...
class Pulse:
    def __init__(
        ...
        if _pulse_instance is not None:
            logger.warning(
                "Pulse already initialized; ignoring re-initialization (new configuration will not be applied)."
            )
...
def get_jsonl_file_exporter():
    ...
    if (
        jsonl_log_file_path is not None
        and jsonl_log_file_path != ""
        and os.path.exists(os.path.dirname(jsonl_log_file_path))
    ):
